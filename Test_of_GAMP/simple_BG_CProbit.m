...
nz = 256;         % Number of output components (dimension of y)
snr = 40;         % SNR in dB. 

% Create a random Gaussian vector
xmean0 = zeros(nx,1);
xvar0 = ones(nx,1);
x0 = xmean0 + sqrt(xvar0/2).*(randn(nx,2)*[1;1i]);
sparseRat = 0.1;
x = x0.*(rand(size(x0))<sparseRat);
...
y_unquantized = A*x + w;
y =  sign(real(y_unquantized)) + 1j* sign(imag(y_unquantized));

% Decide on MAP or MMSE GAMP
map = 0;

% Create an input estimation class corresponding to a Gaussian vector
% inputEst0 = CAwgnEstimIn(xmean0, xvar0, map);
% inputEst = SparseScaEstim(inputEst0, sparseRat*3);
inputEst0 = CAwgnEstimIn(xmean0, xvar0, map, 'autoTune', true);
...
outputEst = CProbitEstimOut(y, 0 + 1j*0, wvar, false);
