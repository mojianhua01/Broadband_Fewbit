function [ ghat_BPDN, time_BPDN] = func_Broadband_Few_Bit_UPA_BPDN( T_cyclic, r_bit, Params)
...
time_BPDN_acc = 0;
for ii=1:1:REPS
   [ghat_BPDN,~,~,info]= spg_bpdn(opA, r, 1.00*sqrt(2* length(r))*sigma, opts);
   time_BPDN_acc = time_BPDN_acc + info.timeTotal; 
end;
time_BPDN = time_BPDN_acc/REPS
% time_BPDN = (cputime -t)/REPS ;
